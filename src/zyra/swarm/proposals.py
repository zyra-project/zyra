...
def _generate_proposal_via_llm(request: ToolProposalRequest) -> ToolProposal | None:
    ...
    stage_meta = stage_catalog.get(request.stage)
    for option in request.options:
        command_meta = stage_meta.get(option) if stage_meta else None
        if not command_meta:
            continue
        option_entries.append(
            {
                ...
                "required_args": _required_args(command_meta),
                "flags": _option_summaries(command_meta),
            }
        )
    if not option_entries:
        return None
    ...
    if request.instructions:
        payload["instructions"] = request.instructions
    system_prompt = (
        "You are a Zyra stage specialist. Choose the best command from the provided options "
        "and return JSON with fields stage_id, command, args, justification. "
        "Use only the listed commands, include required arguments, and avoid placeholder values."
    )
    user_prompt = json.dumps(payload, indent=2, sort_keys=True)
    try:
        raw = client.generate(system_prompt, user_prompt)
...
def _option_summaries(meta: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return sorted(summaries, key=lambda item: item["flag"])
